...
class InformationExtractor:
    ...
    def __init__(self, llm, config: ReaderConfig):
        ...
        self.personal_info_prompt = PromptTemplate(
            template="""
        ...
        self.skills_extraction_prompt = PromptTemplate(
            template="""
        ...
        self.cultural_fit_prompt = PromptTemplate(
            template="""
                        ...
                        for skill_entry in skill_entries:
                            ...
                            if skill_entry:
                                ...
                                if level_match:
                                    ...
                                    skill_name = skill_entry.replace(level_match.group(0), '').strip()
                                    
                                    level = level_mapping.get(level_text, ProficiencyLevel.INTERMEDIATE)
    ...
    def _extract_skills_fallback(self, resume_text: str) -> List[Skill]:
        ...
        technical_keywords = ['python', 'java', 'javascript', 'react', 'sql', 'git', 'aws', 'docker']
            ...
            if ':' in line and '|' in line:
                try:
                    ...
                    if dimension_name in [d.value for d in CulturalDimension]:
                        ...
                        cultural_scores.append(CulturalFitScore(
    ...
    def _extract_cultural_fit_fallback(self) -> List[CulturalFitScore]:
        ...
        for dimension in CulturalDimension:
            default_scores.append(CulturalFitScore(
                dimension=dimension,
                score=60.0,
...
class CompetencyAssessor:
    ...
    def assess_competencies(self, resume_text: str, skills: List[Skill], 
                          work_experience: List[WorkExperience]) -> List[CompetencyAssessment]:
    ...
    def _assess_single_competency(self, competency_key: str, competency_name: str,
                                resume_text: str, skills: List[Skill], 
                                work_experience: List[WorkExperience]) -> CompetencyAssessment:
    ...
    def _parse_competency_assessment(self, competency_name: str, 
                                   llm_response: str) -> CompetencyAssessment:
        ...
        try:
            ...
            evidence = parsed.get('EVIDENCE', '').split(';') if parsed.get('EVIDENCE') else []
            development_areas = parsed.get('DEVELOPMENT_AREAS', '').split(';') if parsed.get('DEVELOPMENT_AREAS') else []
...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indexes
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_name_timestamp ON resume_analyses (full_name, analysis_timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_analysis_id ON personal_info (analysis_id)')
    ...
    def save_analysis_result(self, result: ResumeAnalysisResult) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Save personal info
                self._save_personal_info(cursor, result.analysis_id, result.personal_info)
                ...
                for assessment in result.competency_assessments:
                    self._save_competency_assessment(cursor, result.analysis_id, assessment)
                
                # Save analysis insights
                self._save_analysis_insights(cursor, result.analysis_id, result.strengths, 
                                           result.improvement_areas, result.red_flags)
    ...
    def _save_personal_info(self, cursor, analysis_id: str, info: PersonalInfo):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            analysis_id, info.full_name, info.email, info.phone, info.address,
            info.linkedin, info.github, info.portfolio, info.summary
        ))
    ...
    def _save_education(self, cursor, analysis_id: str, edu: Education):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            analysis_id, edu.degree, edu.major, edu.school, edu.graduation_year,
            edu.gpa, json.dumps(edu.honors), json.dumps(edu.relevant_courses)
        ))
    ...
    def _save_work_experience(self, cursor, analysis_id: str, exp: WorkExperience):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            analysis_id, exp.position, exp.company, exp.start_date, exp.end_date,
            exp.duration_months, json.dumps(exp.responsibilities), 
            json.dumps(exp.achievements), json.dumps(exp.technologies_used)
        ))
    ...
    def _save_skill(self, cursor, analysis_id: str, skill: Skill):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            analysis_id, skill.name, skill.category.value, skill.proficiency_level.value,
            skill.proficiency_score, skill.years_experience, json.dumps(skill.evidence_sources)
        ))
    
    def _save_cultural_fit(self, cursor, analysis_id: str, cultural_fit: CulturalFitScore):
        """Save cultural fit"""
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (
            analysis_id, cultural_fit.dimension.value, cultural_fit.score,
            json.dumps(cultural_fit.evidence), cultural_fit.reasoning
        ))
    
    def _save_competency_assessment(self, cursor, analysis_id: str, assessment: CompetencyAssessment):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            analysis_id, assessment.competency_name, assessment.current_score,
            assessment.potential_score, json.dumps(assessment.evidence),
            json.dumps(assessment.development_areas)
        ))
    
    def _save_analysis_insights(self, cursor, analysis_id: str, strengths: List[str], 
                              improvement_areas: List[str], red_flags: List[str]):
        """Save analysis insights"""
...
class GEvalAssessor:
    ...
    def evaluate_extraction_quality(self, original_text: str, 
                                  extracted_result: ResumeAnalysisResult) -> float:
...
class ReaderWorkflow:
    ...
    def process_resume(self, file_path: str, candidate_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not resume_text or len(resume_text.strip()) < 50:
                return self._create_error_response("File content is empty or too short", processing_steps)
            ...
            personal_info = self.information_extractor.extract_personal_info(resume_text)
            ...
            cultural_fit_scores = self.information_extractor.extract_cultural_fit(resume_text)
            ...
            analysis_results = self._perform_comprehensive_analysis(
                resume_text, skills, work_experience, cultural_fit_scores, competency_assessments
            )
            
            # Create analysis result
            analysis_id = f"{personal_info.full_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            result = ResumeAnalysisResult(
                ...
                career_progression_analysis=analysis_results['career_progression_analysis'],
            ...
            processing_time = (datetime.now() - start_time).total_seconds()
            processing_steps.append(f"Processing completed in {processing_time:.2f} seconds")
            ...
            response = {
                ...
                "extracted_data": {
                    "personal_info": self._serialize_personal_info(result.personal_info),
            ...
            print(f"Resume analysis completed for {result.personal_info.full_name} with {result.confidence_score:.2f} confidence")
            return response
    ...
    def handle_query(self, query: str, file_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _extract_work_experience_simple(self, resume_text: str) -> List[WorkExperience]:
        ...
        job_keywords = ['developer', 'engineer', 'manager', 'analyst', 'intern', 'consultant']
        company_patterns = re.findall(r'(?:at|@)\s+([A-Za-z\s&.]{2,30})', resume_text, re.IGNORECASE)
        ...
        if any(keyword in resume_text.lower() for keyword in job_keywords):
            experiences.append(WorkExperience(
                ...
                company=company_patterns[0] if company_patterns else "Company information pending",
                start_date="To be confirmed",
                end_date="To be confirmed",
                responsibilities=["Responsibility information extracted from resume"],
    ...
    def _perform_comprehensive_analysis(self, resume_text: str, skills: List[Skill],
                                      work_experience: List[WorkExperience],
                                      cultural_fit_scores: List[CulturalFitScore],
                                      competency_assessments: List[CompetencyAssessment]) -> Dict[str, Any]:
        """Perform comprehensive analysis"""
        
        # Calculate overall score
        skill_avg = sum(skill.proficiency_score for skill in skills) / len(skills) if skills else 50
        cultural_avg = sum(score.score for score in cultural_fit_scores) / len(cultural_fit_scores) if cultural_fit_scores else 60
        competency_avg = sum(assessment.current_score for assessment in competency_assessments) / len(competency_assessments) if competency_assessments else 55
        ...
        early_talent_suitability = min(85, overall_score + 10)  # Early talent usually has higher growth potential
        
        # Leadership potential assessment
        leadership_indicators = [
            assessment.current_score for assessment in competency_assessments 
            if 'leadership' in assessment.competency_name.lower() or 
               'team' in assessment.competency_name.lower() or
               'communication' in assessment.competency_name.lower()
        ]
        leadership_potential = sum(leadership_indicators) / len(leadership_indicators) if leadership_indicators else 60
    ...
    def _validate_extraction(self, original_text: str, result: ResumeAnalysisResult) -> Dict[str, Any]:
    ...
    def _create_error_response(self, error_message: str, processing_steps: List[str]) -> Dict[str, Any]:
