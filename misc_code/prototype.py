...
def get_config():
    ...
    return SystemConfig(openai_api_key=api_key, debug_mode=True)
...
def log_agent_activity(agent_name: str, action: str, details: Dict[str, Any] = None):
    ...
    if config.debug_mode:
        ...
        print(
            f"{Fore.RED}[LOG]{Style.RESET_ALL} {Fore.GREEN}[{timestamp}]{Style.RESET_ALL} {agent_name}: {action}"
        )
        if details:
...
class RouterAgent:
    ...
    def classify_query(self, state: MasterAgentState) -> MasterAgentState:
        ...
        log_agent_activity(
            "RouterAgent", "classifying_query", {"query": state["user_query"]}
        )
        ...
        try:
            ...
            if research_score > support_score:
                ...
                query_type = (
                    QueryType.RESEARCH.value
                    if len(query.split()) > 8
                    else QueryType.CUSTOMER_SUPPORT.value
                )
            ...
            log_agent_activity(
                "RouterAgent", "classification_complete", {"query_type": query_type}
            )
...
class ResearchWorkflow:
    ...
    def outliner_agent(self, state: MasterAgentState) -> MasterAgentState:
        ...
        log_agent_activity(
            "OutlinerAgent", "outline_completed", {"sections": len(outline["sections"])}
        )
        return state
    ...
    def researcher_agent(self, state: MasterAgentState) -> MasterAgentState:
        ...
        log_agent_activity(
            "ResearcherAgent", "gathering_information", {"topic": state["topic"]}
        )
...
class SelfRAGAgent:
    ...
    def setup_knowledge_base(self, documents: List[str]):
        ...
        if LANGCHAIN_AVAILABLE:
            ...
            # Mock implementation
            docs = [
                MockDocument(doc, {"source": f"doc_{i}"})
                for i, doc in enumerate(documents)
            ]
            self.vectorstore = MockFAISS(docs, self.embeddings)
    ...
    def generate_response_with_reflection(
        self, state: MasterAgentState
    ) -> MasterAgentState:
        """Generate response using Self-RAG methodology."""
        query = state["user_query"]
        log_agent_activity(
            "SelfRAGAgent", "generating_response_with_reflection", {"query": query}
        )
        ...
        log_agent_activity(
            "SelfRAGAgent", "response_generated", {"confidence": confidence_score}
        )
...
class CustomerSupportWorkflow:
    ...
    def handle_support_query(self, state: MasterAgentState) -> MasterAgentState:
        ...
        query = state["user_query"]
        log_agent_activity(
            "CustomerSupportWorkflow", "processing_support_query", {"query": query}
        )
        ...
        log_agent_activity(
            "CustomerSupportWorkflow", "support_query_processed", {"category": category}
        )
...
class EvaluationSystem:
    ...
    def _evaluate_relevancy(self, query: str, response: str) -> float:
        ...
        if "?" in query and any(
            word in response.lower() for word in ["because", "due to", "result"]
        ):
            basic_relevancy += 0.1
    ...
    def _evaluate_groundedness(self, response: str, documents: List) -> float:
        ...
        doc_content = " ".join(
            [str(getattr(doc, "page_content", "")) for doc in documents]
        ).lower()
        response_lower = response.lower()

        # Count sentence-level overlap
        response_sentences = [
            s.strip() for s in response_lower.split(".") if len(s.strip()) > 10
        ]
    ...
    def _generate_recommendations(self, eval_results: Dict):
        ...
        if eval_results["hallucination_score"] < 0.7:
            recommendations.append(
                "Consider adding more factual verification and source citations"
            )

        if eval_results["relevancy_score"] < 0.7:
            recommendations.append(
                "Improve direct addressing of the user's specific query"
            )

        if eval_results["groundedness_score"] < 0.7:
            recommendations.append(
                "Include more evidence-based information and references"
            )
        ...
        if not recommendations:
            recommendations.append(
                "Response meets quality standards across all metrics"
            )
...
class MasterAgent:
    ...
    def __init__(self):
        try:
            ...
            log_agent_activity(
                "MasterAgent", "initialization_complete", {"status": "ready"}
            )
        except Exception as e:
    ...
    def process_query(
        self, user_query: str, config_dict: Dict[str, Any] = None
    ) -> Dict[str, Any]:
